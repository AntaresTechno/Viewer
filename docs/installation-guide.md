...
| 层 | 技术 | 说明 |
|---|---|---|
| 后端 | Python 3.11–3.13（推荐 **uv + Python 3.12**）、FastAPI、SQLAlchemy 2 (async) + aiosqlite、PyJWT、httpx、lxml/jsonpath-ng、QuickJS | 入口 `app.main:app` |
...
| Python | 3.11–3.13（推荐 3.12） | <https://www.python.org/downloads/> | Windows 安装时勾选 **Add python.exe to PATH**；`build.bat` 会自动经 uv 下载 3.12 |
...
书源规则中的 `@js` / `{{}}` / `jsLib` 固定由 **QuickJS** 执行，不提供其他后端或
运行时切换。`backend/requirements.txt` 会直接安装 `quickjs`；推荐使用 Python 3.12，
当前支持 Python 3.11–3.13。

管理端仪表盘只显示 QuickJS 的安装状态。若显示未安装，请在后端虚拟环境中执行：

```powershell
uv pip install --python .venv\Scripts\python.exe quickjs
```
...
**Q4：书源搜索无结果或 `@js` 规则报错？**
确认 QuickJS 已安装：`pip list | findstr -i quickjs`。若无法安装，请确认使用的是受支持的 Python 3.11–3.13（推荐 3.12）。
