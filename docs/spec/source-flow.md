...
| Rhino JS（`:modules:rhino`） | 一切规则 JS：searchUrl/@js/{{}}/formatJs/preUpdateJs/loginCheckJs/coverDecodeJs/imageDecode | 固定使用 QuickJS；注入 `rhino_compat.js` 提供 `JavaImporter`/`Packages`/`importClass`/`importPackage`/okhttp3/hutool 兼容；注意 Double 整数化输出、`String.format("%.0f")` 语义与 bindings 全集对齐 |
